    ...
    describe('FacetResultsCollection', function () {
      ...
      describe('Server calls', function () {
        ...
        it('makes a facet request for search initiated event', function () {
          ...
          expect(provider.requestJSON.getCall(0).args[0].osParameters.osSearchTerms).toEqual('ice');
          expect(provider.requestJSON.getCall(0).args[0].osFacetFilters).not.toBeDefined();
        });
        ...
        it('uses facet filters for refined search event', function () {
          ...
          expect(provider.requestJSON.getCall(0).args[0].osParameters.osSearchTerms).toEqual('ice');
          expect(provider.requestJSON.getCall(0).args[0].osParameters.osFacetFilters).toEqual({facetParameter: ['Albedo', 'Ice Extent']});
        });
        ...
        describe('Server responses and processing', function () {
          ...
          it('triggers the facets refined event when facets are refined', function () {
            ...
            facetsCollection.onRefinedSearch(fakeSearchParamsModel);
            expect(fakeMediator.callCount).toEqual(1);
            expect(fakeMediator.getCall(0).args[0]).toEqual('search:facetsRefined');
          });
