...
Mount directories are defined in `/config/local_webserver_config.yaml`. By
default, `src/` is mounted to `/`, so to run the app, run `grunt
build:nsidc-dev` or `grunt build:acadis-dev`, then `./run_local_webserver` and
open your browser to http://localhost:8081.

To run using the code generated by the RequireJS optimizer, modify
...
contain endpoints for the services running on `localhost`. To use services
running in a different environment, configure `remote_proxies` appropriately and
start the server with `./run_local_webserver 8081 remote_proxies`.
